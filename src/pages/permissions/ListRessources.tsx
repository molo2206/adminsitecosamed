...
const ListRessources = () => {
	...
	const { handleUpdate, handleDelete, isEdit, selected, isOpen, toggleModal, closeModal } =
		useAuthContext()
	const { data, loading } = useAsync(() => PermissionServices.getPermissions())
	...
	const columns: ReadonlyArray<Column> = [
		...
		},
		{
			...
			width: 400,
			Cell: ({ cell }:any) => (
				<ChangeStatus status={cell?.row?.original?.status} id={cell?.row?.original?.id}/>
			),
		},
	...
	return (
		<>
			...
			<MainModal size={undefined}
				close={closeModal}
				show={isOpen}
				title={`${isEdit ? 'Update' : 'Create'} Ressource`}>
				<Form
				...
				<Col>
					<Card>
						...
						<Card.Header className='d-flex justify-content-between align-items-center'>
							<h4 className="header-title">Ressouces</h4>
							<Button
							onClick={toggleModal}
								style={{
									marginRight: 20,
