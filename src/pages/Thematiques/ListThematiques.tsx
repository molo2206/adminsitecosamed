...
import Thematiques from '@/hooks/Thematiques'
...
const ListThematiques = () => {
	const { createThematiques, loading: loadingForm } = Thematiques()
	...
	const { data, loading } = useAsync(() => ThematiqueServices.getThematiques())
	const validation = useFormik({
		...
		initialValues: {
			value: isEdit
				? showingTranslateValue(selected?.translations, pageLang)?.value
				: '',
		},
	...
	useEffect(() => {
		if (isEdit) {
			// setSelectedType(
			// 	JSON.parse(selected?.types)?.map((item: any) => ({
			// 		label: item,
			// 		value: item,
			// 	}))
			// )
	...
	return (
		<>
			<PageBreadcrumb title="Thematique" subName="Thematique" />
			<MainModal
				size={undefined}
				close={closeModal}
				show={isOpen}
				title={`${isEdit ? t('Update') : t('Create')} ${t('Theme')}`}>
				<Form
...
export default ListThematiques
